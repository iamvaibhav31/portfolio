...
export const formatAchievements = (achievements) => {
  // const pointers = ['🏆', '🥇', '🥈', '🥉'];
  return achievements
    .map(ach => {
      const descriptions = ach.description.map(r => `• ${r}`).join('<br>');

      return `
<div class="mb-2">
<b class="text-primary">${ach.title}</b> 
<span class="text-sm">${descriptions}</span>
</div>`;
    })
    .join('');
};
...
export const commandHandlers = {
  ...
  [commandMap.HELP]: (data) => formatHelp(data),
  [commandMap.ACHIEVEMENTS]: (data) => formatAchievements(data),
